...
class IsotonicRegressionModel (
  ...
  def predict(testData: Double): Double = {
    ...
    def linearInterpolation(x1: Double, y1: Double, x2: Double, y2: Double, x: Double): Double = {
      require(!x1.isNaN && !y1.isNaN && !x2.isNaN && !y2.isNaN && !x.isNaN)
      if y1 == y2 then y1
      else if y1.isInfinite then y1
      else if x2 == x1 then y1
      else if x.isInfinite then
        if y2 > y1 then x
        else if y2 < y1 then -x
        else y1
      else if x2.isInfinite then y2
      else if x1.isInfinite || (x2 - x1).isInfinite then y1
      else if x == x1 then y1
      else 
       y1 + (y2 - y1) * (x - x1) / (x2 - x1)
