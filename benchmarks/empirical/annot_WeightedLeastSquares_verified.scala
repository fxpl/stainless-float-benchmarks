...
private class Aggregator(bSum: Double = 0.0, bbSum: Double = 0.0, wSum: Double = 0.0) extends Serializable {
    ...
    // TO SPECIFY: 158
    def bBar: Double = {
        if wSum.isZero then 0.0 else if bSum.isInfinite then bSum else bSum / wSum
    }.ensuring(!_.isNaN)
    ...
    // TO SPECIFY: 159
    def bbBar: Double = {
         if wSum.isZero then 0.0 else if bbSum.isInfinite then bbSum else bbSum / wSum
    }.ensuring(0d <= _)
    ...
    // TO SPECIFY: 160
    def bStd: Double = {
        if bBar * bBar >= bbBar then 0d else
        math.sqrt(bbBar - bBar * bBar)
    }.ensuring(0d <= _)
}