...
object Variance {
  ...
  def calculate(count: Double, sum: Double, sumSquares: Double): Double = {
    ...
    if sumSquares.isPosInfinity then Double.PositiveInfinity else squaredLoss / count
  }.ensuring(res => res >= 0)
}
...
class VarianceCalculator(stats: Array[Double]){
   ...
   */
  def count: Long = if stats(0) <= Long.MaxValue then stats(0).toLong else Long.MaxValue
