...
object FdLibmBenchmark {
  ...
  object Sin {
    @opaque
    def computeSin(x: Double): Double = {
    ...
    @opaque
    private[FdLibmBenchmark] def __kernel_sin(x: Double, y: Double, iy: Int): Double = {
  ...
  object Cos {
    @opaque
    def computeCos(x: Double): Double = {
      val ix = __HI(x) & EXP_SIGNIF_BITS
      if (ix <= 0x3fe9_21fb) // |x| ~< pi / 4
        __kernel_cos(x, 0.0d)
      else if (ix >= EXP_BITS) // sin(Inf or NaN) is NaN
    ...
    @opaque
    private[FdLibmBenchmark] def __kernel_cos(x: Double, y: Double): Double = {
      require(x.isFinite && -0.7854 <= x && x <= 0.7854)
      require(y.isFinite && -1.1102230246251565E-16 <= y && y <= 1.1102230246251565E-16)
      val ix = __HI(x) & EXP_SIGNIF_BITS
      if (ix < 0x3e40_0000) { // abs(x) < 7.450580596923828125e-9
  ...
  object Tan {
    @opaque
    def computeTan(x: Double): Double = {
      var n = 0
      var ix = 0
      // High word of x.
      ix = __HI(x)
...
//      && ((x.isFinite && -1.0d <= x && x <= 1.0d) ==> (res.isFinite && res.isPositive && 0 <= res && res <= stainless.math.Pi))
  ...
  object Log10 {
    ...
    @opaque
    def computeLog10(x: Double): Double = {
      val hx = __HI(x) // high word of x
      ...
      if (hx < 0x0010_0000) && (((hx & EXP_SIGNIF_BITS) | lx) == 0) then -TWO54 / 0.0
        ...
        if hx2 >= EXP_BITS then
          ...
          val y = (k2 + i).toDouble
          (y * log10_2lo + ivln10 * Log.computeLog(__HI_CHECKED(x2, (hx2 & 0x000f_ffff) | ((0x3ff - i) << 20)))) + y * log10_2hi
    }.ensuring( res =>
  ...
  object Cosh {
    ...
    @opaque
    def computeCosh(x: Double): Double = {
      if x.isNaN then Double.NaN
      else
        // High word of |x|
        ...
        if ix >= EXP_BITS then x * x
          ...
          if ix < 0x3fd6_2e43 then
            ...
            if ix < 0x4036_0000 then
              val t = Exp.computeExp(stainless.math.abs(x))
//              unfold(Exp.computeExp(stainless.math.abs(x)))
              0.5 * t + 0.5 / t
            else
              // |x| in [22, log(maxdouble)] return 0.5*exp(|x|)
              if ix < 0x4086_2E42 then {
//                unfold(Exp.computeExp(stainless.math.abs(x)))
                0.5 * Exp.computeExp(stainless.math.abs(x))
              } else
                // |x| in [log(maxdouble), overflowthreshold]
                val lx = __LO(x)
                if ix < 0x4086_33CE || ((ix == 0x4086_33ce) && (IntHelpers.compareUnsigned(lx, 0x8fb9_f87d) <= 0)) then
                  val w = Exp.computeExp(0.5 * stainless.math.abs(x))
//                  unfold(Exp.computeExp(0.5 * stainless.math.abs(x)))
                  val t = 0.5 * w
                  t * w
                else
...
//        && (!x.isNaN ==> (res >= 1))
  ...
  object Tanh {
    ...
    @opaque
    def computeTanh(x: Double): Double = {
      if x.isNaN then Double.NaN
      else
        // High word of |x|.
        val jx = __HI(x)
        ...
        if ix >= EXP_BITS then
          ...
          if ix < 0x4036_0000 && ix < 0x3c80_0000 then x * (1.0 + x) // tanh(small) = small // |x| < 2**-55
            ...
            val z =
              if ix < 0x4036_0000 then
                if ix >= 0x3ff0_0000 then // |x| >= 1
                  1.0 - 2.0 / (Expm1.computeExpm1(2.0 * stainless.math.abs(x)) + 2.0)
                else
                  val t = Expm1.computeExpm1(-2.0 * stainless.math.abs(x))
                  -t / (t + 2.0)
              else // |x| > 22, return +-1
